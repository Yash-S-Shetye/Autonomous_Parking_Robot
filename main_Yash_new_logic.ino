  ...
  if(!finish){  //gloabal check flag, will set to be true when finish parking
    ...
    while(!localfinish){
      ...
      if(isintersection && c_itsc==0) { //if meet intersection
        ...
        if(rob.isobstacle()==true) {
          while(rob.isobstacle()==true) {};
          endTime = millis();
        }
        time_elapsed = endTime - startTime;
        if(time_elapsed < 100) {
          parkinglot[parkidx]=0;
        }
        else {
          parkinglot[parkidx]=1;
        }
        time_elapsed=0, startTime=0, endTime=0;
      }
        ...
        c_itsc+=1;pre=false;parkidx+=1; startTime=millis();
        if(rob.isobstacle()==true) {
          endTime = millis();
        }
        time_elapsed = endTime - startTime;
      ...
      /*if(!isintersection && c_itsc==1){
        ...
        else if(pre==true && rob.isobstacle()==false){
          endTime=millis();
          pre=false;
        }
      }
